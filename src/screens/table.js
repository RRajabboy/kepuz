...
export default function Table({ fetch, activeFilter }) {

    const { data, changeParams } = fetch

    const navigate = useNavigate()
	const tabBar = async (to) => {
		navigate("/" + to)
		changeParams({ page: to })
	}
    ...
    return <div className="container flex flex-col">
        ...
        <table className="w-full mt-3 relative z-0" cellPadding="5px">
            <thead>
                <tr>
